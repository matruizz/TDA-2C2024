...
//primer elemento del par el el nodo adyacente, el segundo elemento es el indice que le corresponde en la lista ordenada por peso 
//unordered_map<list<pair<ll, ll> > > adj; // adjacency list of graph

//Variables de findBridges
ll timer;
vector<bool> visited;
vector<ll> tin, low;
ll cantComConex;
...
void dfs(ll &v, ll &p, unordered_map<ll, vector<pair<ll, ll> > > &adj, vector<tuple<ll, ll, ll, ll>> &E, ll &k, ll &aristas) {
    ...
    for (pair<ll, ll> to : adj[v]) {
        ...
        if (visited[u]) {
            ...
            dfs(u, v, adj, E, k, aristas);

            low[v] = min(low[v], low[u]);

            if (low[u] > tin[v]){                
                //Si entra aca es puente
...
void find_bridges(unordered_map<ll, vector<pair<ll, ll> > > &adj, vector<tuple<ll, ll, ll, ll> > &E, ll &k, ll &aristas, ll &n) {

    visited.assign(n, false);   //O(n)
    tin.assign(n, -1);          //O(n)
    low.assign(n, -1);          //O(n)
    timer = 0;
    cantComConex = 0;
...
void crearGPrima(vector<tuple<ll, ll, ll, ll> > &E, ll &k, DisjointSet &disSet, ll &aristas, ll &n){
    ...
    //adj.assign(n, {});
    unordered_map<ll, vector<pair<ll, ll> > > adj;    //O(1)

    ll peso = std::get<0>(E[k]);                //O(1)

    while ((k + cantMismoW < E.size()))         //O(cantMismoW)
    {   
        ...
        if ((std::get<0>(E[k + cantMismoW]) == peso))
        {
            ...
            if (repreU == repreV)
            {
                ...
                adj[repreU - 1].push_back(std::make_pair(repreV - 1, cantMismoW));
                adj[repreV - 1].push_back(std::make_pair(repreU - 1, cantMismoW));
            }
    ...
    find_bridges(adj, E, k, aristas, n);
