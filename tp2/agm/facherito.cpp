...
void dfs(ll v, ll p, vector<tuple<ll, ll, ll, ll>> &E, ll &k) {
    ...
    for (pair<ll, ll> to : adj[v]) {
        ...
        if (visited[u]) {
            ...
            if (low[u] > tin[v]){   //Si entra en este if ES PUENTE
...
void find_bridges(vector<tuple<ll, ll, ll, ll> > &E, ll &k, ll &n) {
    ...
    for (ll i = 1; i < (n + 1); ++i) {  //O(m' + n') donde ' indica aristas y nodos de G' es decir con mismo peso
...
void crearGPrima(vector<tuple<ll, ll, ll, ll> > &E, ll &k, DisjointSet &disSet, ll &n){
    ...
    while ((k + cantMismoW) < E.size()) //O(mismoW) que puede ser O(m)
    {
        //Si el peso de la arista actual es igual al peso que se repite
        if ((std::get<0>(E[k + cantMismoW]) == peso))   //O(1)
        {
            ...
            ll repreU = disSet.findSet(u);  //O(1)
            ll repreV = disSet.findSet(v);  //O(1)
            ...
            if (repreU == repreV)
            {
                ...
                adj[repreU].push_back(make_pair(repreV, cantMismoW));   //O(1)
                adj[repreV].push_back(make_pair(repreU, cantMismoW));   //O(1)
            }
...
void kruskal(vector<tuple<ll, ll, ll, ll>> &E, ll &n){
    ...
    sort(E.begin(), E.end());   //O(mlog(m))

    DisjointSet dsu = DisjointSet(n);   //O(n)

    aristas = 0;
    
    ll i;

    for(i = 0; i < E.size(); i++){  //O(m) o O(n) depende de cuantas son ANY
...
int main() {
    ...
    //         peso,  u,  v, orden original en el que se metio
    vector<tuple<ll, ll, ll, ll> > lAristas(m);     //O(m)
    ...
    kruskal(lAristas, n);   //O(m.log(n)) union by rank y path compression
                            //O(m.log(m)) por el sort de las aristas
