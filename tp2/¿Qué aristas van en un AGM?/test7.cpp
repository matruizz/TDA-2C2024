...
void crearGPrima(vector<vector<ll> > &E, ll &k, DisjointSet &disSet, ll &aristas, ll &n){
    ...
    {
        //Si no son ni ANY y tampoco es NONE tienen que ser AT LEAST ONE
...
void kruskal(vector<vector<ll> > &E, ll &n){
    ...
    for(i = 0; i < E.size(); i++){
        ...
        if (i < (E.size() - 1))
        {
            ...
            //Si la arista actual y la siguiente tienen pesos distintos
            if (w1 != w2)
            {
                //SI la arista actual une componenetes conexas distintas
